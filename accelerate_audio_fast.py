...
with gr.Blocks(title="快速对音频进行加速处理") as demo:
    ...
    generate_btn.click(
        fn=accelerate,
        inputs=[gr.Slider(minimum=1.0, maximum=2, show_reset_button=True, step=0.01, label='speed rate',
                          value=1.2)],
        outputs=[audio_output1, audio_output2, audio_output3]
    )

demo.launch(server_port=port, show_error=True, allowed_paths=[desktop_path], inbrowser=True)
